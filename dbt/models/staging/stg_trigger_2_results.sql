...
select
    ...
    try(
        date_parse(
            replace(substr(c1_open_time_utc, 1, 19), 'T', ' '),
            '%Y-%m-%d %H:%i:%s'
        )
    ) as c1_open_time_utc,
    ...
    try(
        date_parse(
            replace(substr(c2_open_time_utc, 1, 19), 'T', ' '),
            '%Y-%m-%d %H:%i:%s'
        )
    ) as c2_open_time_utc,
    ...
    try(
        date_parse(
            replace(substr(c3_open_time_utc, 1, 19), 'T', ' '),
            '%Y-%m-%d %H:%i:%s'
        )
    ) as c3_open_time_utc,
    ...
    try_cast(c1_close_less_than_c2_close as boolean)
        as c1_close_less_than_c2_close,

    try_cast(c2_close_less_than_c3_close as boolean)
    ...
    try(
        date_parse(
            replace(substr(close_lookback_first_candle_open_time_utc, 1, 19), 'T', ' '),
            '%Y-%m-%d %H:%i:%s'
        )
    ...
    try(
        date_parse(
            replace(substr(close_lookback_last_candle_open_time_utc, 1, 19), 'T', ' '),
            '%Y-%m-%d %H:%i:%s'
        )
    ) as close_lookback_last_candle_open_time_utc,

    -- C3 rejection metrics
    try_cast(c3_body_length as double) as c3_body_length,
    try_cast(c3_upper_shadow_length as double) as c3_upper_shadow_length,
    try_cast(c3_upper_shadow_to_body_ratio as double) as c3_upper_shadow_to_body_ratio,
    try_cast(c3_upper_shadow_to_body_min_ratio as double) as c3_upper_shadow_to_body_min_ratio,
    try_cast(c3_upper_shadow_pass as boolean) as c3_upper_shadow_pass,

    try_cast(c3_lower_shadow_length as double) as c3_lower_shadow_length,
    try_cast(c3_lower_shadow_to_body_ratio as double) as c3_lower_shadow_to_body_ratio,
    try_cast(c3_lower_shadow_to_body_max_ratio as double) as c3_lower_shadow_to_body_max_ratio,
    try_cast(c3_lower_shadow_pass as boolean) as c3_lower_shadow_pass,

    -- Volume confirmation
    volume_confirmation_rule,

    try_cast(volume_lookback_candles_max as integer)
        as volume_lookback_candles_max,

    try_cast(volume_lookback_candles_used as integer)
    ...
    try_cast(c1_volume_confirmation_pass as boolean)
        as c1_volume_confirmation_pass,

    try_cast(c2_volume_confirmation_pass as boolean)
    ...
    try_cast(body_lookback_candles_max as integer)
        as body_lookback_candles_max,

    try_cast(body_lookback_candles_used as integer)
    ...
    try_cast(c1_body_confirmation_pass as boolean)
        as c1_body_confirmation_pass,

    try_cast(c2_body_confirmation_pass as boolean)
        as c2_body_confirmation_pass,

    try(
        date_parse(
            replace(substr(body_lookback_first_candle_open_time_utc, 1, 19), 'T', ' '),
            '%Y-%m-%d %H:%i:%s'
        )
    ...
    try(
        date_parse(
            replace(substr(body_lookback_last_candle_open_time_utc, 1, 19), 'T', ' '),
            '%Y-%m-%d %H:%i:%s'
        )
    ) as body_lookback_last_candle_open_time_utc,

    -- S3 lineage
    nullif(trim(trigger_candle_source_s3_key), '')
        as trigger_candle_source_s3_key,

    nullif(trim(recent_activity_s3_key), '')
